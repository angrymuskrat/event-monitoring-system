...
func GeneratePosts(n int) []data.Post {
	...
	for i := 0; i < n; i++ {
		posts[i] = data.Post{ID: RandString(20), Shortcode: RandString(10), ImageURL: RandString(30),
...
func GeneratePostsFast(n int) []data.Post {
	posts := make([]data.Post, n)
	for i := 0; i < n; i++ {
		posts[i] = data.Post{ID: RandString(20), Shortcode: "sfsfhhsgfhsf", ImageURL: "RandString(30)",
